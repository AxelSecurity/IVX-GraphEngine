"""Tests per il provider ctlogs.dev (Certificate Transparency).

Unico provider CT dopo la rimozione di crt.sh (2026-08-27).  Formato
delle risposte conforme al contratto documentato su
...
provider usa l'endpoint pubblico ``https://ctlogs.dev/search`` (stessa
...
class TestQueryCtlogsMocked:
    ...
    async def test_cache_separated_by_mode(self, monkeypatch):
        ...
        la chiave arriva il provider rifà le richieste e vede i sibling
